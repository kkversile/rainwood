...
async function main() {
  ...
  const demoReservations = [
    { reference: 'RW-DEMO-001', guestName: 'Ananya Rao', email: 'ananya.rao@example.com', mobile: '+91 90000 10001', source: 'WEBSITE' as const, status: 'CONFIRMED' as const, paymentStatus: 'PARTIALLY_PAID' as const, syncStatus: 'SYNCED' as const, arrivalOffset: 3, total: 12320, tax: 1320, advance: 6000, balance: 6320, adults: 2, children: 0 },
    { reference: 'RW-DEMO-002', guestName: 'Vikram Menon', email: 'vikram.menon@example.com', mobile: '+91 90000 10002', source: 'DIRECT' as const, status: 'PENDING_PAYMENT' as const, paymentStatus: 'PENDING' as const, syncStatus: 'PENDING' as const, arrivalOffset: 10, total: 12320, tax: 1320, advance: 0, balance: 12320, adults: 2, children: 1 },
    { reference: 'RW-DEMO-003', guestName: 'Meera Shah', email: 'meera.shah@example.com', mobile: '+91 90000 10003', source: 'PHONE' as const, status: 'COMPLETED' as const, paymentStatus: 'PAID' as const, syncStatus: 'SYNCED' as const, arrivalOffset: -20, total: 12320, tax: 1320, advance: 12320, balance: 0, adults: 2, children: 0 },
    { reference: 'RW-DEMO-004', guestName: 'Arjun Kapoor', email: 'arjun.kapoor@example.com', mobile: '+91 90000 10004', source: 'AGENT' as const, status: 'CANCELLED' as const, paymentStatus: 'REFUNDED' as const, syncStatus: 'PENDING' as const, arrivalOffset: 18, total: 12320, tax: 1320, advance: 5000, balance: 7320, adults: 2, children: 0 },
    { reference: 'RW-ARRIVAL-001', guestName: 'Aarav Nair', email: 'arrival.one@rainwood.demo', mobile: '+91 90000 20001', source: 'WEBSITE' as const, status: 'CONFIRMED' as const, paymentStatus: 'PAID' as const, syncStatus: 'SYNCED' as const, arrivalOffset: 0, total: 11000, tax: 1000, advance: 11000, balance: 0, adults: 2, children: 0 },
    { reference: 'RW-ARRIVAL-002', guestName: 'Diya Menon', email: 'arrival.two@rainwood.demo', mobile: '+91 90000 20002', source: 'AGENT' as const, status: 'TENTATIVE' as const, paymentStatus: 'PARTIALLY_PAID' as const, syncStatus: 'SYNCED' as const, arrivalOffset: 0, total: 13200, tax: 1200, advance: 6600, balance: 6600, adults: 2, children: 1 },
    { reference: 'RW-ARRIVAL-003', guestName: 'Kabir Thomas', email: 'arrival.three@rainwood.demo', mobile: '+91 90000 20003', source: 'DIRECT' as const, status: 'CONFIRMED' as const, paymentStatus: 'PENDING' as const, syncStatus: 'PENDING' as const, arrivalOffset: 0, total: 9800, tax: 800, advance: 0, balance: 9800, adults: 1, children: 0 },
  ];
  for (const item of demoReservations) {
    const checkIn = demoDate(item.arrivalOffset);
    const checkOut = demoDate(item.arrivalOffset + 2);
    const existing = await prisma.reservation.findUnique({ where: { reference: item.reference } });
    const reservation = existing ?? await prisma.reservation.create({ data: { reference: item.reference, hotelId: hotel.id, source: item.source, sourceName: 'RainWood demo seed', status: item.status, paymentStatus: item.paymentStatus, syncStatus: item.syncStatus, guestName: item.guestName, email: item.email, mobile: item.mobile, checkIn, checkOut, currency: 'INR', totalAmount: item.total, taxAmount: item.tax, advanceAmount: item.advance, balanceAmount: item.balance, priceSnapshot: [{ roomTypeId: room.id, ratePlanId: plan.id, amount: item.total - item.tax, taxAmount: item.tax }], policySnapshot: { freeCancellationHours: 48, firstNightPenalty: true, seeded: true }, internalRemark: 'Development seed record', createdById: admin.id, lines: { create: { roomTypeId: room.id, ratePlanId: plan.id, checkIn, checkOut, rooms: 1, adults: item.adults, children: item.children, nightlyRate: Math.round((item.total - item.tax) / 2), taxAmount: item.tax, lineTotal: item.total, priceSnapshot: { amount: item.total - item.tax, taxAmount: item.tax }, nights: { create: [item.arrivalOffset, item.arrivalOffset + 1].map((nightOffset) => ({ date: demoDate(nightOffset), rooms: 1, amount: Math.round((item.total - item.tax) / 2), taxAmount: Math.round(item.tax / 2), totalAmount: Math.round(item.total / 2) })) } } } } });
    const paymentAmount = item.reference === 'RW-DEMO-001' ? 6000 : item.reference === 'RW-DEMO-003' ? 12320 : item.reference === 'RW-DEMO-004' ? 5000 : item.reference === 'RW-ARRIVAL-001' ? 11000 : 0;
