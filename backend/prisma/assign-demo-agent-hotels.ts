...
async function main() {
  ...
  for (const item of demoHotels) {
    ...
    await prisma.agentRatePlan.upsert({ where: { agentId_ratePlanId: { agentId: agent.id, ratePlanId: ratePlan.id } }, update: { active: true, pricingMode: 'BASE' }, create: { agentId: agent.id, ratePlanId: ratePlan.id, active: true, pricingMode: 'BASE' } });
