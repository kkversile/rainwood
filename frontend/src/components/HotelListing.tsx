  ...
  return <>
    ...
    {!filteredHotels.length ? <p className="empty">No hotels match the selected filters.</p> : <div className="demoHotelsCards">{filteredHotels.map((hotel) => {
      ...
      return <article className="demoHotelsCard" key={hotel.id}>
        ...
        <div className="demoHotelsCardBody"><h2>{hotel.name}</h2><p>{hotel.description ?? 'A thoughtful RainWood stay with direct reservation support.'}</p><div className="demoHotelsTags"><span className="goldTag">{mealPlans[0] ?? 'Direct'}</span><span>{hotel.rooms?.length ?? 0} room categories</span><span>Open</span></div><div className="demoHotelsCardActions"><Link className="btn" href={`/hotels/${hotel.slug}`}>View Rooms</Link><Link className="demoHotelsBookLink" href={bookingHref(hotel)}>Book direct →</Link></div></div>
      </article>;
    })}</div>}
  </>;
}
