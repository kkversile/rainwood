...
import { agentPaymentConfirmation, aggregatePriceBreakdown } from '../lib/booking-pricing';
...
type AgentTerms = { paymentTerms?: { mode: 'PERCENTAGE' | 'CREDIT'; advancePercent?: number | string | null } | null };
...
const AgentHotelImage = HotelImage;
...
function AgentBookingRail({
  ...
  return <aside className={`agentBookingRail${publicMode ? ' publicBookingRail' : ''}`} aria-label="Booking criteria">
    {publicMode ? <>
      <h2>Choose your stay</h2>
      <p>Select dates and occupancy to see live room availability.</p>
      <label>Hotel<select value={hotelId ?? ''} onChange={(event) => onHotelChange?.(event.target.value)}><option value="">All hotels</option>{hotels.map((hotel) => <option key={hotel.id} value={hotel.id}>{hotel.name} - {hotel.city}</option>)}</select></label>
    </> : onChangeHotel && <button type="button" className="agentChangeHotel" onClick={onChangeHotel}>← Change hotel</button>}
    <div className="publicBookingDates">
      <label>Check-in<input type="date" value={checkIn} onChange={(event) => onCheckIn(event.target.value)} /></label>
      {!publicMode && <label>Nights<input type="number" min="1" max="30" value={nights} onChange={(event) => onNights(Math.max(1, Number(event.target.value) || 1))} /></label>}
      <label>Check-out<input type="date" value={checkOut} onChange={(event) => onCheckOut(event.target.value)} /></label>
    </div>
...
function AgentCheckoutSummary({ selected, checkIn, checkOut, nights, rooms, adults, children, holdTotal, requiredPayment = holdTotal, terms = null, walletBalance, onBack }: { selected: AvailabilityOption; checkIn: string; checkOut: string; nights: number; rooms: number; adults: number; children: number; holdTotal: number; requiredPayment?: number; terms?: AgentTerms | null; walletBalance: number; onBack: () => void }) {
  ...
  return <div className="agentCheckoutSummary">
    ...
    <h2>Cart Details</h2>
    <div className="agentCartTableWrap"><table className="agentCartTable"><thead><tr><th>Price summary</th><th>Amount</th></tr></thead><tbody><tr><td>Room Charges</td><td>{money(price.roomCharges)}</td></tr>{price.extraGuestCharges > 0 && <tr><td>Extra Guest Charges</td><td>{money(price.extraGuestCharges)}</td></tr>}{price.supplementaryCharges.map((charge) => <tr key={charge.id}><td>{charge.name}</td><td>{money(charge.amount)}</td></tr>)}<tr><td>Tax</td><td>{money(price.tax)}</td></tr></tbody><tfoot><tr><td>Grand total</td><td>{money(holdTotal)}</td></tr></tfoot></table></div>
    ...
    <div className="agentPolicySection"><h3>Payment Terms</h3><p>Terms: <b>{terms?.paymentTerms?.mode === 'CREDIT' ? 'Credit' : `${Number(terms?.paymentTerms?.advancePercent ?? 100)}% Advance`}</b></p><p>Booking Total: <b>{money(holdTotal)}</b></p><p>Pay Now: <b>{money(requiredPayment)}</b></p><p>Balance: <b>{money(Math.max(0, holdTotal - requiredPayment))}</b></p><p>Balance Due: <b>At check-in</b></p><p>Wallet Balance: <b>{money(walletBalance)}</b></p><p>Wallet After: <b>{money(Math.max(0, walletBalance - requiredPayment))}</b></p></div>
...
function PublicPriceSummary({ selected, total }: { selected: AvailabilityOption; total: number }) {
  const price = aggregatePriceBreakdown(selected);
  return <dl className="bookingSummary"><div><dt>Room Charges</dt><dd>{money(price.roomCharges)}</dd></div>{price.extraGuestCharges > 0 && <div><dt>Extra Guest Charges</dt><dd>{money(price.extraGuestCharges)}</dd></div>}{price.supplementaryCharges.map((charge) => <div key={charge.id}><dt>{charge.name}</dt><dd>{money(charge.amount)}</dd></div>)}<div><dt>Tax</dt><dd>{money(price.tax)}</dd></div><div><dt>Grand Total</dt><dd>{money(total)}</dd></div></dl>;
}
...
function AgentGuestForm({ guest, updateGuest, submitGuest, busy, holdTotal, requiredPayment = holdTotal, credit = false }: { guest: GuestForm; updateGuest: GuestUpdater; submitGuest: (event: FormEvent) => void; busy: boolean; holdTotal: number; requiredPayment?: number; credit?: boolean }) {
  return <form className="agentAbadGuestForm" onSubmit={submitGuest}>
    ...
    <div className="agentGuestGrid agentGuestGridTwo"><label>GST / Tax Number (Optional)<input value={guest.gstin} onChange={(event) => updateGuest('gstin', event.target.value)} /></label><label>Bill To Company Name *<input value={guest.companyName} onChange={(event) => updateGuest('companyName', event.target.value)} required /></label></div>
    ...
    <div className="agentPayBar"><span>Pay now <b>{money(requiredPayment)}</b> · Balance <b>{money(Math.max(0, holdTotal - requiredPayment))}</b></span><button className="btn" disabled={busy}>{busy ? 'Confirming...' : credit ? 'Confirm booking on credit' : `Confirm booking · Pay ${money(requiredPayment)}`}</button></div>
  </form>;
}
...
export function BookingFlow({ agentMode = false }: { agentMode?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    apiRequest<AgentTerms>('/agents/me/profile').then(setAgentTerms).catch((reason: Error) => setError(reason.message));
  }, [agentMode]);
  ...
  const paymentPercent = Number(agentTerms?.paymentTerms?.advancePercent ?? 100);
  const requiredPayment = agentMode && agentTerms?.paymentTerms?.mode === 'CREDIT' ? 0 : Math.round(holdTotal * paymentPercent) / 100;
  ...
  async function createHold(option: AvailabilityOption) {
    setError('');
    const displayedRequired = agentMode && agentTerms?.paymentTerms?.mode === 'CREDIT' ? 0 : Math.round(Number(option.total) * paymentPercent) / 100;
    if (agentMode && wallet && walletBalance < displayedRequired) {
      setError(`Wallet balance is ${money(walletBalance)} but this booking needs ${money(displayedRequired)} now. Recharge your wallet before booking.`);
      return;
    }
    setBusy(true);
    try {
  ...
  return <div className={`page bookingWorkspace${agentMode ? ' agentBookingFlow' : ' publicBookingFlow'}`}>
    ...
    {step === 'guest' && hold && selected && agentMode && <section className="agentAbadCheckout"><AgentCheckoutSummary selected={selected} checkIn={checkIn} checkOut={checkOut} nights={nights} rooms={rooms} adults={adults} children={children} holdTotal={holdTotal} requiredPayment={requiredPayment} terms={agentTerms} walletBalance={walletBalance} onBack={() => setStep('room')} /><AgentGuestForm guest={guest} updateGuest={updateGuest} submitGuest={submitGuest} busy={busy} holdTotal={holdTotal} requiredPayment={requiredPayment} credit={agentTerms?.paymentTerms?.mode === 'CREDIT'} /></section>}
    ...
    {step === 'confirmation' && reservation && <section className="formCard confirmation"><span className="status ok">Confirmed</span><h2>Booking confirmed</h2><p>Thank you, {reservation.guestName}. Your reference is <b>{reservation.reference}</b>.</p><p>{reservation.hotel.name} - {reservation.checkIn} to {reservation.checkOut}</p><p>Payment status: <b>{reservation.paymentStatus}</b></p>{agentMode && <p className="notice">{agentPaymentConfirmation(reservation)}</p>}<div className="actions"><a className="btn" href={`/booking/confirmation?reference=${encodeURIComponent(reservation.reference)}`}>View confirmation</a>{agentMode && <a className="btn secondary" href="/agent/bookings">View my bookings</a>}</div></section>}
  </div>;
}
