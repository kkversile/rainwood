            ...
            <div className="reviewColumns">
              ...
              <section className="reviewListCard">
                ...
                <div className="reviewTableWrap"><table className="reviewTable"><thead><tr><th>S.NO</th><th>RATING</th><th>DESCRIPTION</th><th>CREATED ON</th><th>ACTIONS</th></tr></thead><tbody>
                  {visibleReviews.length ? visibleReviews.map((review, index) => <tr key={review.id}><td>{reviewFirst + index}</td><td><span className="reviewStars" aria-label={`${review.rating} out of 5 stars`}>{[1, 2, 3, 4, 5].map((star) => <span key={star} className={star <= review.rating ? "filled" : ""}>★</span>)}</span></td><td>{review.description}</td><td>{new Date(review.createdAt).toLocaleDateString()}</td><td><div className="tableActionGroup"><button type="button" className="reviewEditButton" onClick={() => { setEditingReviewId(review.id); setReviewForm({ rating: String(review.rating), description: review.description }); }}>Edit</button><button type="button" className="reviewDeleteButton" onClick={() => setReviewDeleteTarget(review)}>Delete</button></div></td></tr>) : <tr><td colSpan={5} className="reviewEmpty">No data available in table</td></tr>}
                </tbody></table></div>
