...
export default function SiteSettingsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (file) {
        ...
        value = `/files/public/${stored.id}`;
      }
      if (value && !/^https?:\/\//i.test(value) && !value.startsWith('/files/public/')) throw new Error('Logo URL must be an uploaded image or start with http:// or https://.');
  ...
  return <AdminLayout title="Site Settings"><section className="masterPanel"><div className="listToolbar"><div><span>Branding</span><h2>RainWood logo</h2><p>Upload a logo or provide an image URL. It will be displayed in the public header.</p></div></div><form className="formCard masterForm" onSubmit={submit}><label>Logo image URL<input type="text" value={logoUrl} onChange={(event) => { setLogoUrl(event.target.value); setFile(null); }} placeholder="https://example.com/rainwood-logo.png" /></label><label>Or upload logo<input type="file" accept="image/jpeg,image/png" onChange={(event) => { setFile(event.target.files?.[0] ?? null); if (event.target.files?.[0]) setLogoUrl(''); }} /><small className="mutedText">JPEG or PNG, maximum 5 MB.</small></label>{logoUrl && <div className="hotelImagePreview"><b>Preview</b><div className="hotelImageThumbs"><div className="hotelImageThumb"><img src={apiAssetUrl(logoUrl)} alt="Logo preview" /></div></div></div>}{error && <p className="error" role="alert">{error}</p>}{message && <p className="notice" role="status">{message}</p>}<div className="actions"><button className="btn" disabled={busy}>{busy ? 'Saving...' : 'Save logo'}</button><button className="smallBtn" type="button" disabled={busy} onClick={() => { setLogoUrl(''); setFile(null); }}>Remove logo</button></div></form></section></AdminLayout>;
}
