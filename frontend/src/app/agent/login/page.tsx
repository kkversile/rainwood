...
export default function AgentLogin() {
  ...
  return <main className="login"><form className="formCard" onSubmit={submit}><h1>Agent Login</h1><p>Sign in to manage your RainWood agent bookings. New registrations require admin approval.</p>{params.get('pending') === '1' && <p className="notice" role="status">Registration submitted. An administrator must approve your account before login.</p>}{error && <p className="error" role="alert">{error}</p>}<label>Email<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoComplete="username" required /></label><label>Password<input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" required /></label><button className="btn full" disabled={busy}>{busy ? 'Signing in...' : 'Sign in'}</button><p><Link href="/agent/register">Create an agent account</Link></p></form></main>;
}
